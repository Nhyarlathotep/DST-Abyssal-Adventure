...
function AddTile(id, numerical_id, name, specs, minispecs, isflooring)
    ...
    if GROUND[id] ~= nil then
        modassert(GROUND[id] == numerical_id, ("GROUND.%s already exists: %s (%s), and differs from intended %s."):format(id, tostring(GROUND_NAMES[GROUND[id]]), tostring(GROUND[id]), tostring(numerical_id)))
...
function ChangeTileTypeRenderOrder(tiletypeid, targettiletypeid, moveafter)
    ...
    if tiletypeid == targettiletypeid then
        moderror(("[ChangeTileTypeRenderOrder(%s,%s,%s)] Trying to move ground %s (%s) in relation to itself."):format(tostring(tiletypeid), tostring(targettiletypeid), tostring(moveafter), tostring(tiletypeid), tostring(GROUND_LOOKUP[tiletypeid])))
        return nil
    end
    ...
    if idx == nil then
        moderror(("[ChangeTileTypeRenderOrder(%s,%s,%s)] Tile type %s (%s) not found."):format(tostring(tiletypeid), tostring(targettiletypeid), tostring(moveafter), tostring(tiletypeid), tostring(GROUND_LOOKUP[tiletypeid])))
        return nil
    end
    ...
    if targetidx == nil then
        moderror(("[ChangeTileTypeRenderOrder(%s,%s,%s)] Tile type %s (%s) not found."):format(tostring(tiletypeid), tostring(targettiletypeid), tostring(moveafter), tostring(targettiletypeid), tostring(GROUND_LOOKUP[targettiletypeid])))
        return nil
    end
